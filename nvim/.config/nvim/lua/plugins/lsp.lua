return {
  ...
  },
  {
    ...
    config = function()
      ...
      -- Note: These configurations are declarative and won't start servers immediately

      -- Table of LSP server configurations
      -- mason_name: optional, if the Mason package name differs from the LSP server name
      -- filetypes: list of filetypes that should activate this LSP
      local servers = {
      ...
      local ensure_installed = vim.tbl_values(vim.tbl_map(function(config)
        return config.mason_name
      end, servers))
